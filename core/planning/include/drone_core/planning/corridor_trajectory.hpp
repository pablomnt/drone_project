...
// segments whose Bezier control points are confined to a per-segment free box
// (so the whole trajectory provably stays inside the corridor — the Bezier hull
// contains the curve) and whose velocity/acceleration/jerk control points
// respect per-axis limits. The decision variables stay in the monomial basis
// (same snap cost block as MinSnapTrajectory); the Bernstein map only shapes
// the linear constraint rows, so the solution drops straight into
// common::Trajectory for the flatness mapper. Solved as a QP with OSQP (C API,
// confined to the .cpp).
...
public:
  ...
  // Inner solve for a fixed time allocation: one degree-7 segment per box,
  // rest-to-rest from `start` (current drone position, must be in boxes[0]) to
  // `goal` (truncation endpoint, must be in boxes.back()), C0-C4 continuous at
  // junctions. Segment i is confined to boxes[i]; times[i] is its duration.
  // Returns false when the QP is infeasible (corridor too tight / times too
  // short for the limits) or the solver fails; then `out` is untouched. On
  // success fills `out` (t0 left to the caller) and, if given, `cost_out` with
  // the snap objective value (the outer time search minimises this).
  bool solveQP(const Eigen::Vector3d& start, const Eigen::Vector3d& goal,
               const std::vector<double>& times, const std::vector<Box>& boxes,
  ...
  // / vmax + buffer, slightly generous so the search starts feasible and
  // shrinks — instead of the old sqrt heuristic. waypoints are the resampled
  // corridor waypoints (waypoints.size() == boxes.size() + 1); the trajectory
  // runs rest-to-rest from waypoints.front() to waypoints.back(), free within
  // the boxes in between. Returns false when no feasible time allocation was
  // found (the final QP at the best times fails).
